...
if __name__ == '__main__':
    ...
    if args.cuda:
        ...
        torch.cuda.set_device(rank % gpu_count + 4)
    ...
    #--------------------------------------------------------------------------
    ####7777#### net = model_loader.load(args.dataset, args.model, args.model_file)
    vpmodel = np.load('/disk/student/adhara/Spring2022/DENISE-Black-Edition/vpmodels.npy')
    ...
    #####7777#####    net = nn.DataParallel(net, device_ids=range(torch.cuda.device_count()))
    ...
    # load directions
    ####7777 #### d = net_plotter.load_directions(dir_file)
