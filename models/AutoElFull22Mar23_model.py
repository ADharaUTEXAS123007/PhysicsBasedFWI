...
class AutoElFull22Mar23Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='AutoElFull22Mar23',
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
           ...
           #self.optimizer_G = torch.optim.LBFGS(
            ...
            #    [param for name, param in self.netG.named_parameters() if 'Rho' in name], lr=opt.lr, amsgrad=True)
            #self.optimizer_G2 = torch.optim.Adam(
            #    [param for name, param in self.netG.named_parameters() if 'Rho' not in name], lr=0.005)
    ...
    def forward(self,epoch1,lstart,freq,idx,it,mop):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        #netin1 = self.real_A[:, :, 1:800:2, :]
        if (epoch1 == 1):
            self.latent = torch.ones(1,1,1,1)
        [self.fake_Vp,self.fake_Vs,self.fake_Rho, self.grad,self.latent,self.vp_grad,self.vs_grad,self.rho_grad,self.loss_D_MSE] = self.netG(self.real_B,self.real_A,lstart,epoch1,self.latent,self.real_C,self.real_D,freq,idx,it,mop)  # G(A)
        self.real_Vp = torch.unsqueeze(self.real_B[:,0,:,:],1)
        self.real_Vs = torch.unsqueeze(self.real_B[:,1,:,:],1)
        self.real_Rho = torch.unsqueeze(self.real_B[:,2,:,:],1)
    ...
    def forwardT(self):
        ...
        num_shots = 90
        idx = torch.arange(num_shots)
        [self.VpT,self.VsT,self.RhoT,self.gradT,self.latentT,_,_,_,_] = self.netG(self.real_B,self.real_A,False_lstart,False_epoch,self.latentT,self.real_C,self.real_D,freq,idx,1,1)  # G(A)
    ...
    def backward_G11(self, epoch1, batch, lstart, initerror, currenterror, idx):
        ...
        self.loss_VP = self.criterionMSE(self.real_Vp, self.fake_Vp)
    ...
    def compute_loss_only(self):
        ...
        lossMSE = self.criterionMSE(
            self.fake_BT, self.real_B[:,0:3,:,:])/(diff_size[0]*diff_size[1]*diff_size[2]*diff_size[3])
