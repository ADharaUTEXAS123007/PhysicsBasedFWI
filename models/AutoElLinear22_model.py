...
class AutoElLinear22Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='AutoElLinear22',
    ...
    def backward_G11(self, epoch1, batch, lstart, initerror, currenterror):
        ...
        #self.grad = self.grad/torch.max(self.grad.abs())
            ...
            #################
            ###8### self.vp_grad = torch.unsqueeze(self.vp_grad,0)
            ###8### self.vp_grad = self.vp_grad.cuda(self.fake_Vp.get_device())
            ###8### self.fake_Vp.backward(self.vp_grad,retain_graph=True)
            #self.fake_Vs.retain_grad()
            #self.fake_Vp.retain_grad()
            #self.fake_Rho.retain_grad()
            
            ###8### self.vs_grad = torch.unsqueeze(self.vs_grad,0)
            ###8### self.vs_grad = self.vs_grad.cuda(self.fake_Vs.get_device())
            ###8### self.fake_Vs.backward(self.vs_grad,retain_graph=True)
            #self.fake_Vs.retain_grad()
            #self.fake_Vp.retain_grad()
            #self.fake_Rho.retain_grad()
            ...
            ###8### self.rho_grad = torch.unsqueeze(self.rho_grad,0)
            ###8### self.rho_grad = self.rho_grad.cuda(self.fake_Rho.get_device())
            ###8### self.fake_Rho.backward(self.rho_grad)
