    ...
    def init_func(m):  # define the initialization function
        ...
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):
            if init_type == 'normal':
                ...
                init.kaiming_uniform_(m.weight.data, a=0.0, mode='fan_out',nonlinearity='relu')
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = Auto88_Net(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'Auto':
        ...
        net = UnetMarmousiPre_Net(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'Auto21':
    ...
    elif netG == 'AutoElLinear22':
        net = AutoElLinear22_Net(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    ...
    elif netG == 'AutoElFull22Mar23':
        net = AutoElFullMarmousi23Mar22_Net(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
        ...
        net = AutoElFullRhoScaleMarmousiMar22_Net(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'AutoElBPRhoScaleMar22':
        net = AutoElBPRhoScaleMarmousiMar22_Net(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
...
class unetConv10(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        super(unetConv10, self).__init__()
        # Kernel size: 3*3, Stride: 1, Padding: 1
        if is_batchnorm:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, (5, 3), (1,1), (2,1)),
                                       nn.BatchNorm2d(out_size),
                                       nn.LeakyReLU(0.1))
            #self.conv2 = nn.Sequential(nn.Conv2d(out_size, out_size, 3, 1, 1),
            #                           nn.BatchNorm2d(out_size),
...
class Auto88_Net(nn.Module):
    def __init__(self,outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(Auto88_Net, self).__init__()
        self.is_deconv     = False
        self.in_channels   = outer_nc
...
class UnetMarmousiPre_Net(nn.Module):
    def __init__(self,outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(UnetMarmousiPre_Net, self).__init__()
        self.is_deconv     = False
        self.in_channels   = outer_nc
        self.is_batchnorm  = True
        self.n_classes     = 1
...
class AutoElMarmousi22_Net(nn.Module):
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq, idx, it):
        ...
        filen = './marmousiEl9Mar2/vpmod' + str(epoch1) + '.npy' #switch on for physics based fwi         
        np.save(filen, vpst)  #switch on physics based fwi
        
        filen = './marmousiEl9Mar2/vsmod' + str(epoch1) + '.npy' #switch on for physics based fwi     
        np.save(filen, vsst)  #switch on physics based fwi
        
        filen = './marmousiEl9Mar2/rhomod' + str(epoch1) + '.npy' #switch on for physics based fwi     
...
class AutoElLinear22_Net(nn.Module):
    def __init__(self,outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(AutoElLinear22_Net, self).__init__()
        self.is_deconv     = False
        self.in_channels   = outer_nc
        self.is_batchnorm  = True
        self.n_classes     = 1
    ...
    def forward(self, inputs1, inputs2, lstart, epoch1, latentI, lowf, inputs3, freq, idx, it):
        ...
        vs1    = torch.unsqueeze(lowf[:,1,:,:],1) + vs1
        #rho1   = torch.unsqueeze(lowf[:,2,:,:],1) + 0.1*rho1
        rho1   = torch.unsqueeze(inputs1[:,2,:,:],1)
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq, idx, it):
        ...
        #d.forward(model, src, rec)
        #os.system('mpirun -np 4 hello')
        filen = './marmousiEl9Mar4/vpmod' + str(epoch1) + '.npy' #switch on for physics based fwi         
        np.save(filen, vpst)  #switch on physics based fwi
        
        filen = './marmousiEl9Mar4/vsmod' + str(epoch1) + '.npy' #switch on for physics based fwi     
        np.save(filen, vsst)  #switch on physics based fwi
        
        filen = './marmousiEl9Mar4/rhomod' + str(epoch1) + '.npy' #switch on for physics based fwi     
        np.save(filen, rhost)  #switch on physics based fwi
        ...
        #d.add_fwi_stage(fc_low=0.0, fc_high=int(epoch1/10)+1.0)
        #d.add_fwi_stage(fc_low=0.0, fc_high=30.0)
        d.add_fwi_stage(fc_high=10,inv_rho_iter=100000)
        # if ((epoch1 >= 0) and (epoch1 <=100 )):
        #     d.add_fwi_stage(fc_low=0.0, fc_high=2.0)
        # #     #print(f'Stage {i+1}:\n\t{d.fwi_stages[i]}\n')
        # elif ((epoch1 >= 101) and (epoch1 <=200)):
...
class AutoElMarmousi22N_Net(nn.Module):
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq):
        ...
        #d.add_fwi_stage(fc_low=0.0, fc_high=int(epoch1/10)+1.0)
        #d.add_fwi_stage(fc_low=0.0, fc_high=30.0)
        d.add_fwi_stage(fc_low=0.0, fc_high=20.0, inv_rho_iter=0)
        # if ((epoch1 >= 0) and (epoch1 <=100 )):
        #     d.add_fwi_stage(fc_low=0.0, fc_high=2.0)
        # #     #print(f'Stage {i+1}:\n\t{d.fwi_stages[i]}\n')
        # elif ((epoch1 >= 101) and (epoch1 <=200)):
...
class AutoElFullMarmousi23Mar22_Net(nn.Module):
    def __init__(self,outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(AutoElFullMarmousi23Mar22_Net, self).__init__()
        self.is_deconv     = False
        self.in_channels   = outer_nc
        self.is_batchnorm  = True
        self.n_classes     = 1
        ...
        self.down1   = unetDown(221, filters[0], self.is_batchnorm)
        #self.dropD1   = nn.Dropout2d(0.025)
        self.down2   = unetDown(filters[0], filters[1], self.is_batchnorm)
        #self.dropD2   = nn.Dropout2d(0.025)
        self.down3   = unetDown(filters[1], filters[2], self.is_batchnorm)
        ...
        #self.decoder_input1 = nn.Linear(filters[2]*125*26, latent_dim) #for marmousi 151x200
        #self.decoder_input = nn.Linear(latent_dim, filters[2]*500*102) #for marmousi 151x200
        self.decoder_input1 = nn.Linear(filters[3]*157*6, latent_dim) #for marmousi 101x101
        #self.decoder_input = nn.Linear(latent_dim, filters[3]*100*26) #for marmousi 101x101
        #self.decoder_input1 = nn.Linear(filters[1]*100*18, latent_dim) #for marmousi 101x101
        self.decoder_input = nn.Linear(latent_dim, filters[3]*75*50) #for marmousi 101x101
        #self.decoder_inputRho = nn.Linear(latent_dim, 1*300*100)
    ...
    def forward(self, inputs1, inputs2, lstart, epoch1, latentI, lowf, inputs3, freq, idx, it, mop):
        ...
        vp1[(torch.unsqueeze(inputs1[:,0,:,:],1)==1496.0)] = 1496.0
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq, idx, it, mop):
        ...
        def copyshot(id1, value, mop):   
            ...
            fo = 'cp /disk/student/adhara/marine2/sutdir'+str(mop+1)+'/seis_p.su.shot'+str(id1+1)+ ' ' + '/disk/student/adhara/marine2/sudir1/.'
...
class AutoElFullRhoMarmousiMar22_Net(nn.Module):
    ...
    def forward(self, inputs1, inputs2, lstart, epoch1, latentI, lowf, inputs3, freq, idx, it):
        ...
        #############################7########## rho1f    = 10.381522*rho1f + 7.465008
        ##########7#############   vp1f = 73.130*vp1f + 0.267
        ########7#############     vs1f = 43.86*vs1f + 0.1542
        ##########7############     rho1f = 15.447317*rho1f + 9.41e-06
        ###vp1    = self.vp(torch.unsqueeze(f1[:,0,:,:],1))
        ###vs1    = self.vs(torch.unsqueeze(f1[:,1,:,:],1))
        #rho1   = self.rho(f13)
        #vp1     = f11
        ...
        for i in range(396):
            dind[0,0,:,i] = torch.arange(170)

        vp1    = torch.unsqueeze(lowf[:,0,:,:],1) + vp1f
        vs1    = torch.unsqueeze(lowf[:,1,:,:],1) + vs1f
        ##rem### rho1   = torch.unsqueeze(lowf[:,2,:,:],1) + 0.0020*(0.5*rho1f + 0.5*(0.17*vp1+168.4))
        rho1   = torch.unsqueeze(lowf[:,2,:,:],1) + 0.002*(0.5*rho1f + 0.5*(0.17*vp1+168.4))
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq, idx, it):
        ...
        #d.add_fwi_stage(fc_low=0.0, fc_high=int(epoch1/10)+1.0)
        #d.add_fwi_stage(fc_low=0.0, fc_high=30.0)
        d.add_fwi_stage(fc_low=4.0,fc_high=5.0)
        # if ((epoch1 >= 0) and (epoch1 <=100 )):
        #     d.add_fwi_stage(fc_low=0.0, fc_high=2.0)
        # #     #print(f'Stage {i+1}:\n\t{d.fwi_stages[i]}\n')
        # elif ((epoch1 >= 101) and (epoch1 <=200)):
...
class AutoElFullRhoScaleMarmousiMar22_Net(nn.Module):
    def __init__(self,outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
    ...
    def forward(self, inputs1, inputs2, lstart, epoch1, latentI, lowf, inputs3, freq, idx, it):
        ...
        vp1    = torch.unsqueeze(lowf[:,0,:,:],1) + vp1f
        vs1    = torch.unsqueeze(lowf[:,1,:,:],1) + vs1f
        rho1   = torch.unsqueeze(lowf[:,2,:,:],1) + 0.05*rho1f
        #rho1   = torch.unsqueeze(lowf[:,1,:,:],1)  + 0.1*rho1f
...
class AutoElBPRhoScaleMarmousiMar22_Net(nn.Module):
    def __init__(self,outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(AutoElBPRhoScaleMarmousiMar22_Net, self).__init__()
        self.is_deconv     = False
        self.in_channels   = outer_nc
        self.is_batchnorm  = True
        self.n_classes     = 1
    ...
    def prop(self, vp1, vs1, rho1, true, epoch1, freq, idx, it):
        ...
        denise_root = '/disk/student/adhara/WORK/DeniseFWI/virginFWI/DENISE-Black-Edition/'
        d = api.Denise(denise_root,verbose=1)
        d.save_folder = '/disk/student/adhara/SEAMN2/'
        d.set_paths()
        ...
        depth_rec = 23*20.  # receiver depth [m]
        ######depth_rec = 80. #simple_model
        ...
        for i in range(0,tshots):
            print("idx :", idx[i])
            copyshot(idx[i],i)
        d.DATA_DIR = '/disk/student/adhara/SEAMN2/su1/seisT'
        d.SEIS_FILE_VX = 'su1/seisT_x.su'
        d.SEIS_FILE_VY = 'su1/seisT_y.su'
        ...
        d.VPUPPERLIM = 4510.00
        d.VPLOWERLIM = 1500.0
        d.VSUPPERLIM = 2591.95450
        d.VSLOWERLIM = 0.0
        #d.RHOUPPERLIM = 2589.0
        #d.RHOLOWERLIM = 1009.0
